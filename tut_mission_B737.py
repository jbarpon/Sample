...
def full_setup():
    ...
    vehicle  = vehicle_setup()  # vehicle_setup is below
    configs  = configs_setup(vehicle)
    # What is vehicle_stup() and configs_setup(), physically what does this do?
    CG_Trial = CG_Locator(vehicle)
    print(CG_Trial)

    # What does this do? analyses_setup() is next  function below
...
def analyses_setup(configs):
    ...
    # What is SUAVE.Analyses.Analysis.Container()
    analyses = SUAVE.Analyses.Analysis.Container()

    # Build a base analysis for each configuration. Here the base analysis is always used, but
    # this can be modified if desired for other cases.
    # This checks for every tag and config in config.items, what is tag and config physically?
    for tag,config in configs.items():
        analysis = base_analysis(config) # base_analysis() is the next defined function below
...
def base_analysis(vehicle):
    ...
    # ------------------------------------------------------------------     
    analyses = SUAVE.Analyses.Vehicle() # What does SUAVE.Analyses.Vehicle() do

    # What do all the SUAVE.Analyses do? How do I open it?
    # What about the Weights, Aerodynamics, Stability, Energy, etc. is being analysed?\
    # Are we just appending values to the analyses variable, but the math is elsewhere? (i.e. not under base_analysis?)
...
# Is there a differene between having a Vehicle.py file than having a vehicle_setup under the main file?
def vehicle_setup():
...
def CG_Locator(vehicle):
    ...
    Main_Wing_Locations = np.array([0])
    Horizontal_Tail_Locations = np.array([0])
    Vertical_Tail_Locations = np.array([0])
    Fuselage_Locations = np.array([0])
    Propulsor_Locations = np.array([0])
    ...
    for part in vehicle.keys():
        ...
        # Wing Handler
        if isinstance(part,C.Wings):

            if part.sweeps.leading_edge == None:
                part.sweeps.leading_edge = convert_sweep(part, old_ref_chord_fraction=0.25, new_ref_chord_fraction=0.0)

            for wing_type in part.keys():
                print('below is wing type')
                print(wing_type)

                if isinstance(wing_type,C.Wings.Main_Wing):
                    # Does this have to be wing.mass_properties.center_of_gravity or is it part.mass_properties etc. etc.
        ...
        # Propulsor Handler
        elif isinstance(part,C.Propulsors):  
            part.mass_properties.center_of_gravity[0][0] = part.engine_length*0.5
            Propulsor_Locations = np.append(Propulsor_Locations,part.mass_properties.center_of_gravity[0][0])
        
        # Fuselage handler
        elif isinstance(part,C.Fuselages):  
            part.mass_properties.center_of_gravity[0][0] = 0.45*part.lengths.total
            Fuselage_Locations = np.append(Fuselage_Locations,part.mass_properties.center_of_gravity[0][0])

    # end for-loop
    
    # Calculate Length of CG Location Arrays. Subtract 1 from length due to the first entry being 0, only used to initialize
    # Need to check that that length of arrays is not zero. 
    Total_Aircraft_CG = 0

    if (len(Main_Wing_Locations) - 1) > 1:
        Main_Wing_CG_Array_Length = len(Main_Wing_Locations) - 1
        Main_Wing_CG = (0.3/Main_Wing_CG_Array_Length)*np.sum(Main_Wing_Locations)  
        Total_Aircraft_CG +=  Main_Wing_CG

    if (len(Horizontal_Tail_Locations) - 1) > 1:
        Horizontal_Tail_Array_Lengths = len(Horizontal_Tail_Locations) - 1
        Horizontal_Tail_CG = (0.05/Horizontal_Tail_Array_Lengths)*np.sum(Horizontal_Tail_Locations) 
        Total_Aircraft_CG +=  Horizontal_Tail_CG

    if (len(Vertical_Tail_Locations) - 1) > 1:
        Vertical_Tail_Array_Length = len(Vertical_Tail_Locations) - 1
        Vertical_Tail_CG = (0.05/Vertical_Tail_Array_Length)*np.sum(Vertical_Tail_Locations)
        Total_Aircraft_CG += Vertical_Tail_CG

    if (len(Fuselage_Locations) - 1) > 1:
        Fuselage_Array_Length = len(Fuselage_Locations) - 1
        Fuselage_CG = (0.45/Fuselage_Array_Length)*np.sum(Fuselage_Locations)
        Total_Aircraft_CG += Fuselage_CG

    if (len(Propulsor_Locations) - 1) > 1:
        Propulsor_Array_Length = len(Propulsor_Locations) - 1
        Propulsor_CG = (0.1/Propulsor_Array_Length)*np.sum(Fuselage_Locations)
        Total_Aircraft_CG  += Propulsor_CG

    # Misc items are 5% of total weight
